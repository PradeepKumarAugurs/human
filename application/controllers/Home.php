...
class Home extends CI_Controller {
    ...
    public  function  get_id_card()
    {
        ...
        $this->load->view('general/id_card');
        $this->load->view('general/footer'); 
    }

    public  function  find_id_card()
    {
       $row_id=$this->input->post('id'); 
       $get_card = $this->AdminModel->get_data_by_column('letest_news','news_tag',$row_id); 
       if(count($get_card))
       {
          // echo $get_card[0]['news_pdf'];  die;
            $file=$get_card[0]["news_pdf"]; 
            echo $result='<label class="text-center text-success">Result Found...................</label>
                <br/>
                <a href="'.base_url("upload/news_file/".$file).'" target="_blank">
                    <img src="http://localhost/human_right/frontend/images/pic_hrpws/id-card-2.jpg"  style="height:150px;" class="hovercalss"></a>';
       }
       else
       {
         echo $result='<label class="text-center text-danger">No Result Found. Enter A Valid ID!!..</label>
                '; 
       }
    }   
