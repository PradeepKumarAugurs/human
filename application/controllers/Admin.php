...
class Admin extends CI_Controller {
    ...
    public function add_gallery() {  //error_reporting(1);
        if($this->session->userdata('id')) { //print_r($_POST);exit;
            ...
            $data['all_category'] = $this->AdminModel->get_all_category();
            ...
            if($this->input->post('submit') == 'Submit') {
                ...
                $this->load->view('school/add_gallery',$data);
...
/* Add Category */
    public  function add_category()
    {
        if($this->session->userdata('id'))
        {
            ...
            $data['all_category'] = $this->AdminModel->get_all_category();
            $this->load->view('school/header',$data);
            $this->load->view('school/add_category',$data);
            $this->load->view('school/footer',$data);
        } 
        else 
        {
    ...
    {
        $this->form_validation->set_rules('category','Category','required');//print_r($_POST);exit;
        if($this->form_validation->run() == FALSE){
            $this->session->set_flashdata('msg','<div class="alert alert-danger error_list"><a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><strong>Wrong! </strong>'.strip_tags(validation_errors()).'</div>');
        ...
        {
            $insert_data['category']=$this->input->post('category')?$this->input->post('category'):'';
            $insert_res=$this->AdminModel->insert_data('category',$insert_data);
            $this->session->set_flashdata('msg','<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><strong>New  Category  </strong>Added.</div>');
            redirect('admin/add_category');
        }
    }
    ...
    public  function  delete_category()
    {
           ...
           $delete_res=$this->AdminModel->delete_row('category','id',$row_id);
           if($delete_res)
           {
            echo '200';
           }
    ...
    public function add_gallery__()
{
    ...
    public function gallery_list(){
        ...
        foreach($query as $r){
            ...
            $row[] = $i;
            $row[] = $r->image_category;
            $row[] = '<img style="height:70px;width:70px;" src="'.$r->image_url.'"/>';
            $edit = '<button type="button" onClick="Delete('.$r->id .');" name="delete" data-id="' . $r->id . '" class="btn btn-danger delete"><i class="ti-trash"></i></button>
                    ';
            $row[] = $edit;
            $data[] = $row;
            ...
            if($this->input->post('submit') == 'Submit') {
                ...
                if($this->form_validation->run() == FALSE){
                    ...
                    if ($_FILES["news_file"]["name"] != ""){
                        ...
                        $config['allowed_types'] = 'pdf|png|jpg|jpeg|gif|doc';
                        $config['max_size'] = 100000;
                        $config['file_name'] = uniqid().".".pathinfo($_FILES["news_file"]["name"],PATHINFO_EXTENSION);
                        ...
                        if($result) {
                            $this->session->set_flashdata('msg','<div class="alert alert-success error_list"><a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><strong>Success! </strong>ID Card Added Successfully.</div>');
                            redirect('admin/letest_news');
                        } else {
                            $this->session->set_flashdata('msg','<div class="alert alert-danger error_list"><a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a><strong>Error! </strong>Something Went Wrong. Please Try Again.</div>');
                            redirect('admin/letest_news');
